...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp) {
  ...
  if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET) //没有成功初始化的前一个状态就是NO_IMAGES_YET
    ...
    mCurrentFrame = Frame(
        mImGray,
        timestamp,
        mpORBextractorLeft,     //正常运行的时的ORB特征点提取器，提取指定数目特征点 NOTE: why left???
        mpORBVocabulary,
        mK,
        mDistCoef,
        mbf,
...
void Tracking::Track() {
  ...
  if (mState == NOT_INITIALIZED) {
    ...
    else  MonocularInitialization();  //单目初始化
    ...
    if (!mbOnlyTracking) {
      ...
      if (mState == OK) {
        ...
        // 局部建图线程则可能会对原有的地图点进行替换.在这里进行检查    xzf: 为什么会被替换？
        CheckReplacedInLastFrame();

        // Step 2.2 运动模型是空的或刚完成重定位，跟踪参考关键帧；否则恒速模型跟踪
        // 第一个条件,如果运 动模型为空,说明是刚初始化开始，或者已经跟丢了
      ...
      else bOK = Relocalization(); // 如果跟踪状态不成功,那么就只能重定位了  // BOW搜索，EPnP求解位姿
    }
    else {// Localization Mode: Local Mapping is deactivated
      // Step 2：只进行跟踪tracking，局部地图不工作
      if (mState == LOST) bOK = Relocalization(); // Step 2.1 如果跟丢了，只能重定位
      else {
        ...
        if (!mbVO) {  // xzf：false是跟踪正常
          ...
          if (bOKMM && !bOKReloc) {// 恒速模型成功、重定位失败
            // 重新使用之前暂存的恒速模型结果
          ...
          else if (bOKReloc)  mbVO = false; // 只要重定位成功整个跟踪过程正常进行（重定位与跟踪，更相信重定位） xzf: 纯定位模式，更相信重定位
          //有一个成功我们就认为执行成功了
          bOK = bOKReloc || bOKMM;
    } } }
    ...
    if (!mbOnlyTracking)
      ...
      // 重定位成功 xzf: TrackLocalMap可以用于重定位？
    ...
    if (bOK) {
      ...
      for (int i = 0; i < mCurrentFrame.N; i++) {
        ...
        if (pMP)
          if (pMP->Observations() < 1) {  // xzf: 没观测到
    ...
    if (mState == LOST)
      if (mpMap->KeyFramesInMap() <= 5) { //如果地图中的关键帧信息过少的话,直接重新进行初始化了
  ...
  if (!mCurrentFrame.mTcw.empty()) {
    ...
    //保存各种状态
    mlRelativeFramePoses.push_back(Tcr);                // xzf：reference to current
    mlpReferences.push_back(mpReferenceKF);             // xzf：参考关键帧，跟当前帧共视点最多的关键帧
    mlFrameTimes.push_back(mCurrentFrame.mTimeStamp);   // xzf：时间戳
    mlbLost.push_back(mState == LOST);                  // xzf：状态
  } else {
...
void Tracking::MonocularInitialization() {
  ...
  if (!mpInitializer) {
    ...
    if (mCurrentFrame.mvKeys.size() > 100) {
      ...
      //  xzf 把mCurrentFrame深拷贝给分别是mInitialFrame，因为还没有mpInitializer
      mInitialFrame = Frame(mCurrentFrame);
      // 用当前帧更新上一帧 // TODO: xzf 为啥要重复执行两遍Frame(mCurrentFrame)？
      mLastFrame = Frame(mCurrentFrame);
      ...
      // xzf 因为mInitialFrame和mCurrentFrame相等（深拷贝）
...
void Tracking::CheckReplacedInLastFrame() {
  for (int i = 0; i < mLastFrame.N; i++) {
    ...
    if (pMP) {
      ...
      if (pRep) mLastFrame.mvpMapPoints[i] = pRep;  //然后替换一下
    }
  }
}
...
void Tracking::UpdateLastFrame() {
  ...
  // ref_keyframe 到 lastframe的位姿变换  xzf：Tlr是要从右往左读，即reference到last的变换
  ...
  // 如果上一帧为关键帧，或者单目的情况，则退出  xzf：为什么单目不要，因为没有深度？
  ...
  // Step 2.1：得到上一帧中具有有效深度值的特征点（不一定是地图点）  // xzf:有效深度值是指非负
  ...
  // Step 2.2：从中找出不是地图点的部分  xzf: 作为临时地图点，只用于跟踪
  int nPoints = 0;
  for (size_t j = 0; j < vDepthIdx.size(); j++) {
    ...
    // 如果这个点对应在上一帧中的地图点没有,或者创建后就没有被观测到,那么就生成一个临时的地图点  xzf:为啥没被观测到还要？
    MapPoint *pMP = mLastFrame.mvpMapPoints[i];
    if (!pMP) bCreateNew = true;  // xzf: 当前特征点是否存在于 地图点 中，不存在的话则后续需要新建一个临时地图点
    ...
    if (bCreateNew) {
      ...
      // 反投影到世界坐标系中
      cv::Mat x3D = mLastFrame.UnprojectStereo(i);  // xzf：反投影，从2d->3d
      ...
      mlpTemporalPoints.push_back(pNewMP);  // xzf: 再记一次方便后面删它
      nPoints++;
    } else  nPoints++;// 因为从近到远排序，记录其中不需要创建地图点的个数

    // Step 2.4：如果地图点质量不好，停止创建地图点
    // 停止新增临时地图点必须同时满足以下条件：
    // 1、当前的点的深度已经超过了设定的深度阈值（35倍基线）
    // 2、nPoints已经超过100个点，说明距离比较远了，可能不准确，停掉退出
    if (vDepthIdx[j].first > mThDepth && nPoints > 100) // xzf TODO: 每个循环都有加nPoints，为啥不用j？
      break;
  }
}
...
bool Tracking::TrackWithMotionModel() {
  ...
  // Step 2：根据之前估计的速度，用恒速模型得到当前帧的初始位姿。    // xzf: mVelocity=Tcl上一帧last到当前帧current， mTcw上一帧的位姿，所以得到当前帧的位姿
  ...
  // 设置特征匹配过程中的搜索半径
  int th;
  if (mSensor != System::STEREO)  th = 15;//单目  xzf：因为单目的误差大一些
  else  th = 7;//双目
  ...
  if (mbOnlyTracking) { // 纯定位模式下：如果成功追踪的地图点非常少,那么这里的mbVO标志就会置位
    mbVO = nmatchesMap < 10;
    return nmatches > 20;
  }
...
bool Tracking::TrackLocalMap() {
  ...
  for (int i = 0; i < mCurrentFrame.N; i++) {
    if (mCurrentFrame.mvpMapPoints[i]) {
      ...
      if (!mCurrentFrame.mvbOutlier[i]) { // xzf：不属于outlier
        // 找到该点的帧数mnFound 加 1
        mCurrentFrame.mvpMapPoints[i]->IncreaseFound();
        //查看当前是否是在纯定位过程
        if (!mbOnlyTracking) {
          // 如果该地图点被相机观测数目nObs大于0，匹配内点计数+1
          // nObs： 被观测到的相机数目，单目+1，双目或RGB-D则+2 // xzf:双目+2是因为特征点提取的时候要求左右都能看到
          if (mCurrentFrame.mvpMapPoints[i]->Observations() > 0)  mnMatchesInliers++;
        } else  mnMatchesInliers++; // 记录当前帧跟踪到的地图点数目，用于统计跟踪效果
      }
...
bool Tracking::NeedNewKeyFrame() {
  ...
  int nRefMatches = mpReferenceKF->TrackedMapPoints(nMinObs); // xzf: 观测次数足够多 的 地图点 的数量
  ...
  int nNonTrackedClose = 0;  //双目或RGB-D中没有跟踪到的近点  xzf: 近点是啥？
  ...
  if ((c1a || c1b || c1c) && c2) {
    ...
    if (bLocalMappingIdle)  return true;  //可以插入关键帧
    else {
      ...
      if (mSensor != System::MONOCULAR) {
        ...
        if (mpLocalMapper->KeyframesInQueue() < 3)  return true;  //队列中的关键帧数目不是很多,可以插入
        else                                        return false; //队列中缓冲的关键帧数目太多,暂时不能插入
      } else
        //对于单目情况,就直接无法插入关键帧了
        //? 为什么这里对单目情况的处理不一样?
        //回答：可能是单目关键帧相对比较密集
  ...
  } else  return false; //不满足上面的条件,自然不能插入关键帧
}
...
void Tracking::CreateNewKeyFrame() {
  ...
  if (mSensor != System::MONOCULAR) {
    ...
    for (int i = 0; i < mCurrentFrame.N; i++) {
      float z = mCurrentFrame.mvDepth[i];
      if (z > 0)  vDepthIdx.push_back(make_pair(z, i)); // 第一个元素是深度,第二个元素是对应的特征点的id
    }

    if (!vDepthIdx.empty()) {
      // Step 3.2：按照深度从小到大排序  xzf:越近越准
      sort(vDepthIdx.begin(), vDepthIdx.end());
...
void Tracking::SearchLocalPoints() {
  ...
  for (vector<MapPoint *>::iterator vit = mCurrentFrame.mvpMapPoints.begin(), vend = mCurrentFrame.mvpMapPoints.end();
    ...
    MapPoint *pMP = *vit;
    if (pMP) {
      if (pMP->isBad()) *vit = static_cast<MapPoint *>(NULL); // xzf：保证有效
      else {
        // 更新能观测到该点的帧数加1(被当前帧观测了)
        pMP->IncreaseVisible();
        // 标记该点被当前帧观测到
        pMP->mnLastFrameSeen = mCurrentFrame.mnId;
        // 标记该点在后面搜索匹配时不被投影，因为已经有匹配了  xzf: 因为之前恒速模型、重定位匹配过了
        pMP->mbTrackInView = false;
  } } }
  ...
  if (nToMatch > 0) {
    ...
    int th = 1;
    if (mSensor == System::RGBD)  th = 3; //RGBD相机输入的时候,搜索的阈值会变得稍微大一些
...
void Tracking::UpdateLocalKeyFrames() {
  ...
  // Step 2.1 类型1：能观测到当前帧地图点的关键帧作为局部关键帧 （将邻居拉拢入伙）（一级共视关键帧）  xzf: 即能直接看到当前帧地图点的关键帧
  for (map<KeyFrame *, int>::const_iterator it = keyframeCounter.begin(), itEnd = keyframeCounter.end(); it != itEnd;
    ...
    // 表示它已经是当前帧的局部关键帧了，可以 防止重复添加 局部关键帧
    pKF->mnTrackReferenceForFrame = mCurrentFrame.mnId;   // xzf: 防止重复添加
  }
  ...
  for (vector<KeyFrame *>::const_iterator itKF = mvpLocalKeyFrames.begin(), itEndKF = mvpLocalKeyFrames.end();
    ...
    const vector<KeyFrame *> vNeighs = pKF->GetBestCovisibilityKeyFrames(10);   // xzf：共视程度最高的10帧
    ...
    for (vector<KeyFrame *>::const_iterator itNeighKF = vNeighs.begin(), itEndNeighKF = vNeighs.end();
      ...
      if (!pNeighKF->isBad()) {
        ...
        if (pNeighKF->mnTrackReferenceForFrame != mCurrentFrame.mnId) {
          ...
          //? 找到一个就直接跳出for循环？ // xzf：可能找到最高的共视帧就好
          break;
    } } }

    // 类型3:将一级共视关键帧的子关键帧作为局部关键帧（将邻居的孩子们拉拢入伙）  xzf: 当前帧必须是关键帧，才有子关键帧
    ...
    if (pParent) {
      ...
      if (pParent->mnTrackReferenceForFrame != mCurrentFrame.mnId) {
        ...
        //! 感觉是个bug！如果找到父关键帧会直接跳出整个循环   // xzf: 我觉得也是个bug！
        break;
  } } }
          ...
          if (nadditional + nGood >= 50) {
            ...
            if (nGood > 30 && nGood < 50) {
              ...
              nadditional = matcher2.SearchByProjection(
                  ...
                  3,                      //新的窗口阈值，会乘以金字塔尺度
                  64);                    //匹配的ORB描述子距离应该小于这个阈值   xzf: 前面BA优化过，更加准确，所以可以缩小搜索范围
