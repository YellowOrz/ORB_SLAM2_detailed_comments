...
int ORBmatcher::SearchByBoW(KeyFrame *pKF, Frame &F, vector<MapPoint *> &vpMapPointMatches) {
  ...
  while (KFit != KFend && Fit != Fend) {
    ...
    if (KFit->first == Fit->first) {
      ...
      const vector<unsigned int> vIndicesKF = KFit->second; // xzfKFit
      ...
      for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++) {
        ...
        for (size_t iF = 0; iF < vIndicesF.size(); iF++) {
          ...
          // 如果地图点存在，说明这个点已经被匹配过了，不再匹配，加快速度 xzf：？？？为啥会存在？
...
int ORBmatcher::SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2, cv::Mat F12,
  ...
  // Step 1 计算KF1的相机中心在KF2图像平面的二维像素坐标   xzf：其实是极点（像素坐标）
  // KF1相机光心在世界坐标系坐标Cw
  ...
  while (f1it != f1end && f2it != f2end) {
    // 如果f1it和f2it属于同一个node节点才会进行匹配，这就是BoW加速匹配原理  xzf：正向索引表
    if (f1it->first == f2it->first) {
      ...
      for (size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
        ...
        if (pMP1) continue;   // xzf：存在就跳过

        // 如果mvuRight中的值大于0，表示是双目，且该特征点有深度值
        const bool bStereo1 = pKF1->mvuRight[idx1] >= 0;

        if (bOnlyStereo)  if (!bStereo1)  continue;   // xzf：为啥不用&&？
        ...
        for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
          ...
          if (bOnlyStereo)  if (!bStereo2)  continue;
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th) {
  ...
  for (int i = 0; i < nMPs; i++) {
    ...
    if (bestDist <= TH_LOW) {
      ...
      if (pMPinKF){
        // 如果最佳匹配点有对应有效地图点，选择被观测次数最多的那个替换   xzf: 融合
        if (!pMPinKF->isBad()) {
          if (pMPinKF->Observations() > pMP->Observations())  pMP->Replace(pMPinKF);
          else  pMPinKF->Replace(pMP);
      } }
      else {  // 如果最佳匹配点没有对应地图点，添加观测信息          xzf: 新增
...
int ORBmatcher::SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches12,
  ...
  for (int i1 = 0; i1 < N1; i1++) {
    ...
    // 该特征点存在对应的地图点 或者 该特征点已经有匹配点了，跳过
    ...
    int bestDist = INT_MAX;
    int bestIdx = -1;
    // Step 3.4：遍历所有候选特征点，寻找最佳匹配点（并未使用次佳最佳比例约束） xzf:可能
    for (vector<size_t>::const_iterator vit = vIndices.begin(), vend = vIndices.end(); vit != vend; vit++) {
      const size_t idx = *vit;
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono) {
  ...
  // Step 2 计算当前帧和前一帧的平移向量
  //当前帧的相机位姿 xzf：世界到当前，p'=Rcw*p+tcw
  const cv::Mat Rcw = CurrentFrame.mTcw.rowRange(0, 3).colRange(0, 3);
  const cv::Mat tcw = CurrentFrame.mTcw.rowRange(0, 3).col(3);

  //当前相机坐标系到世界坐标系的平移向量  xzf：当前到世界，p = Rcw^-1*p' = Rcw^-1*tcw = Rwc*p'+twc
  ...
  // 当前帧相对于上一帧相机的平移向量 xzf：Tlc=Tlw*Twc
  const cv::Mat tlc = Rlw * twc + tlw;  // = - Rlw * Rcw.t() * tcw + tlw

  // 判断前进还是后退 xzf:在z方向上的前进or后退，用来判断金字塔的错位配准
  ...
  for (int i = 0; i < LastFrame.N; i++) {
    ...
    if (pMP) {
      if (!LastFrame.mvbOutlier[i]) { // xzf: PNP的操作，先将
        // 对上一帧有效的MapPoints投影到当前帧坐标系  xzf：准确来说是变换，不是投影吧？
        ...
        if (bForward) // 前进,则上一帧兴趣点在所在的尺度nCurOctave>=nLastOctave
